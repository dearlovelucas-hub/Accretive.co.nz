...
test("unsubscribed user now receives full output", async () => {
  ...
  const job = await createCompletedDraftJobForTest({
    ...
    dealInfo: "Deal details for preview gating test"
  });
  ...
  assert.equal(result.isPaywalled, false);
...
test("bypass attempt against output payload also returns full output for unsubscribed users", async () => {
  await setSubscriptionForUser("user_lucas", {
    plan: "free",
    status: "inactive"
  });
